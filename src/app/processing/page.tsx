      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-[#E8ECF4] tracking-tight">Document Processing</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <LiveIndicator />
                <span className="text-[#4A5268]">|</span>
                <span className="text-xs text-[#7A8499]">Real pipeline — real SEAP document</span>
              </div>
            </div>
          </div>

          <p className="text-[#7A8499] max-w-2xl leading-relaxed">
            This is the actual processing pipeline used on every SEAP tender. Not a simulation — the example below shows
            real output from tender <span className="text-[#E8ECF4] font-mono text-sm">SCN1040947</span> fetched on 2026-03-24.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-[#13161C] border border-[#3B7BF5]/30 rounded-2xl p-6 mb-8"
        >
          <div className="flex items-start justify-between gap-4 mb-5">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-mono text-[#4A5268]">{realExample.noticeNo}</span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#3B7BF5]/10 text-[#60A5FA] border border-[#3B7BF5]/20 uppercase">LIVE DATA</span>
              </div>
              <h2 className="text-base font-semibold text-[#E8ECF4] leading-tight">{realExample.title}</h2>
            </div>
            <div className="flex-shrink-0 text-right">
              <p className="text-2xl font-bold text-[#22C55E]">{realExample.matchScore}%</p>
              <p className="text-[10px] text-[#7A8499]">match score</p>
            </div>
          </div>

          <div className="space-y-2 mb-4">
            {realExample.files.map((file, i) => (
              <div key={i} className="flex items-center justify-between bg-[#0C0E12] border border-[#252A35] rounded-lg px-3 py-2.5">
                <div className="flex items-center gap-2.5">
                  <FileText className={`w-4 h-4 ${fileColors[file.type] || "text-[#4A5268]"}`} />
                  <div>
                    <p className="text-xs font-medium text-[#E8ECF4]">{file.name}</p>
                    <p className="text-[10px] text-[#7A8499]">{file.size}</p>
                  </div>
                </div>
                <span className={`text-[10px] font-semibold uppercase ${fileStatusColors[file.status] || "text-[#7A8499]"}`}>
                  {file.status}
                </span>
              </div>
            ))}
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-[#252A35]">
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-[#22C55E]" />
              <span className="text-xs text-[#7A8499]">All signatures verified</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Zap className="w-4 h-4 text-[#F59E0B]" />
              <span className="text-xs text-[#7A8499]">Processed in {realExample.processingTime}</span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-3 mb-10">
          <h2 className="text-xs uppercase tracking-[0.15em] text-[#7A8499] font-medium mb-5">
          ...
          {pipeline.map((step, i) => {
            ...
            return (
              <motion.div
                ...
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1 + 0.07 * i, ease: "easeOut" }}
                className="bg-[#13161C] border border-[#252A35] rounded-xl overflow-hidden"
              >
                <button
                  onClick={() => setExpandedStep(isExpanded ? null : step.step)}
                  className="w-full flex items-center gap-4 p-4 hover:bg-[#1A1E27] transition-colors duration-200 text-left"
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    step.type === "ai"
                      ? "bg-[#3B7BF5]/10 border border-[#3B7BF5]/20"
                      : "bg-[#60A5FA]/10 border border-[#60A5FA]/20"
                  }`}>
                    <Icon className={`w-4 h-4 ${step.type === "ai" ? "text-[#3B7BF5]" : "text-[#60A5FA]"}`} />
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[10px] text-[#4A5268] font-mono">Step {step.step}</span>
                      <span className={`text-[10px] uppercase font-semibold px-1.5 py-0.5 rounded ${
                        step.type === "ai"
                          ? "bg-[#3B7BF5]/10 text-[#60A5FA]"
                    ...
                    <p className="text-sm font-semibold text-[#E8ECF4]">{step.title}</p>
                  </div>

                  {isExpanded ? (
                    <ChevronDown className="w-4 h-4 text-[#4A5268] flex-shrink-0" />
                  ) : (
                    <ChevronRight className="w-4 h-4 text-[#4A5268] flex-shrink-0" />
                  )}
                </button>
                ...
                <AnimatePresence>
                  {isExpanded && (
                    ...
                    >
                      <div className="px-4 pb-4 border-t border-[#252A35] pt-4 space-y-4">
                        <p className="text-sm text-[#7A8499] leading-relaxed">{step.description}</p>

                        <div className="grid sm:grid-cols-2 gap-4">
                          <div>
                            <h4 className="text-[10px] text-[#7A8499] uppercase tracking-wider mb-2">Implementation Details</h4>
                            <div className="space-y-1.5">
                              {step.details.map((d, j) => (
                                <div key={j} className="flex items-start gap-2">
                                  <span className="w-1 h-1 rounded-full bg-[#4A5268] mt-1.5 flex-shrink-0" />
                                  <p className="text-xs text-[#7A8499] font-mono">{d}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div>
                            <h4 className="text-[10px] text-[#7A8499] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                              <Terminal className="w-3 h-3" /> Live Log Output
                            </h4>
                            <div className="bg-[#0C0E12] border border-[#252A35] rounded-lg p-3 space-y-1">
                              {step.log.map((line, j) => (
        ...
        >
          <h2 className="text-xs uppercase tracking-[0.15em] text-[#7A8499] font-medium mb-5">Technology Stack</h2>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { name: "OpenSSL 3.x", role: ".p7s / .p7m verification", color: "text-[#3B7BF5]", bg: "bg-[#3B7BF5]/10 border border-[#3B7BF5]/20" },
              { name: "pdfminer.six", role: "Text PDF extraction", color: "text-[#A855F7]", bg: "bg-[#A855F7]/10 border border-[#A855F7]/20" },
              { name: "Tesseract OCR", role: "Scanned PDF + Romanian", color: "text-[#F59E0B]", bg: "bg-[#F59E0B]/10 border border-[#F59E0B]/20" },
              { name: "Claude AI", role: "Brief + scoring + risks", color: "text-[#22C55E]", bg: "bg-[#22C55E]/10 border border-[#22C55E]/20" },
            ].map(tech => (
              <div key={tech.name} className={`rounded-xl p-4 ${tech.bg}`}>
                <p className={`text-sm font-bold mb-1 ${tech.color}`}>{tech.name}</p>
                <p className="text-xs text-[#7A8499]">{tech.role}</p>
              </div>
            ))}
          </div>

          <div className="mt-5 pt-5 border-t border-[#252A35]">
            <div className="flex items-center gap-2 mb-3">
              <AlertTriangle className="w-4 h-4 text-[#F59E0B]" />
              <h3 className="text-xs font-semibold text-[#7A8499] uppercase tracking-wider">Why This Matters</h3>
            </div>
